...
class DistilConfig(TrainingArguments):
    """Configuration class for the DistilTrainer (Self-Distillation)."""
    ...
    remove_unused_columns: Optional[bool] = field(
        default=False,
        metadata={
            "help": "Whether to only keep the column 'prompt' in the dataset. In self-distillation we rely on "
            "additional columns (e.g. 'teacher_prompt'), so this defaults to `False`."
        },
    )
    ...
    alpha: float = field(
        default=1.0,
        metadata={
            "help": "Alpha coefficient. If `0.0`, the forward KL is used. If `1.0` (default), the reverse KL is used. If anything in between, the Jensen-Shannon Divergence is used."
        },
    )
    ...
    mask_truncated_completions: bool = field(
        ...
        metadata={
            ...
            "being incorrectly penalized and introducing noise during training."
        },
    )
